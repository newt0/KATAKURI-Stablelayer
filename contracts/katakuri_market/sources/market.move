module katakuri_market::market {
    ...
    public fun create_market<T>(
        ...
        // Clone question for the event before it's moved into Market
    ...
    /// Get the current LMSR probability of an outcome (returns Q64)
    public fun get_outcome_probability<T>(market: &Market<T>, outcome_index: u64): u128 {
        ...
        let q = market.q_values[outcome_index];
        let exp_q = math::exp(math::div(math::to_q64(q), b_q64));
        math::div(exp_q, sum_exp)
    }

    /// Estimate the cost to buy `shares` of an outcome (before fees)
    public fun estimate_buy_cost<T>(market: &Market<T>, outcome_index: u64, shares: u64): u64 {
        ...
        let mut q_copy = market.q_values;
        let q_ref = &mut q_copy[outcome_index];
        *q_ref = *q_ref + shares;
        let c_new = calculate_cost(&q_copy, market.b);
        math::from_q64(c_new - c_old)
    }

    /// Get market info: (question, outcomes, b, resolved, winner)
